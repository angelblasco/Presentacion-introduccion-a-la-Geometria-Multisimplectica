...
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Denominamos:\\
$\mathcal{Z}^{l}(M) \Longrightarrow$ Espacio de l-formas cerradas en M.\\
$\mathcal{H}^{k-1}(M) \Longrightarrow$ Conjunto de las formas Hamiltonianas de orden $k-1$.
$\mathcal{V}^{H}_{1}(M) \Longrightarrow$ Conjunto de los campos vectoriales Hamiltonianos.\\
\pause
\begin{exampleblock}{Importante:}
De la no-degeneración de $\omega$ se puede deducir que para un $\alpha \in \mathcal{H}^{k-1}(M)$, el correspondiente campo vectorial Hamiltoniano esta unicamente determinado $X_{\alpha}$.
\end{exampleblock}
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
