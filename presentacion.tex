...
La geometría multisimpléctica puede entenderse como una generalización de la geometría simpléctica al contexto de las teorías de campos clásicas. Mientras que una variedad simpléctica está equipada con una 2-forma cerrada y no degenerada, la geometría multisimpléctica considera variedades provistas de una 
(k+1)-forma cerrada y no degenerada, adecuadas para describir sistemas con varias variables independientes.
\end{frame}
%----------------------------------------
\begin{frame}{Introducción.}
Este marco geométrico surge de manera natural en el estudio de formulaciones covariantes de las ecuaciones de Euler–Lagrange, especialmente en el contexto de los fibrados de jets y los fibrados multimomento. La forma multisimpléctica codifica la dinámica del sistema de forma intrínseca, extendiendo el formalismo Hamiltoniano clásico a dimensiones superiores.
\end{frame}
%----------------------------------------
\begin{frame}{Introducción.}
Los orígenes de la geometría multisimpléctica se remontan a los trabajos de \textbf{De Donder} y \textbf{Weyl} sobre formulaciones Hamiltonianas de teorías de campos. Su desarrollo geométrico moderno fue impulsado por contribuciones fundamentales de \textbf{Tulczyjew, Kijowski}, y posteriormente sistematizado por \textbf{Gotay, Isenberg} y \textbf{Marsden}, entre otros.
\end{frame}
%----------------------------------------
\begin{frame}{Introducción.}
Entre algunos de los trabajos mas importantes de los anteriores autores que han influido en el posterior desarrollo de la geometría multisimpléctica podemos destacar:	
\begin{itemize}
	\item T. De Donder, \textit{Théorie Invariantive du Calcul des Variations}, Gauthier-Villars, 1935.
	\item H. Weyl, \textit{Geodesic fields in the calculus of variations}, Ann. of Math., 1935.
	\item J. Kijowski, \textit{A finite-dimensional canonical formalism in the classical field theory}, Commun. Math. Phys., 1974.
	\item M. J. Gotay, J. Isenberg, J. E. Marsden, \textit{Momentum maps and classical relativistic fields}, Part I \& II, 1998.
\end{itemize}
\end{frame}
%----------------------------------------
...
\pause
\begin{block}{Definición:}
Un campo vectorial $X$ en $M$ es locálmente Hamiltoniano si $\mathcal{L}_{X}\omega =0$
\end{block}
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Sea un campo vectorial $X$ con una k-forma $i_{x}\omega$ exacta, es decir, $i_{X}\omega = d \alpha$ para alguna $(k-1)$-forma $\alpha$ de $M$, entonces:
\begin{block}{Definición:}
\begin{itemize}
	\item $X$ es Hamiltoniano.
	\item $\alpha$ es la forma Hamiltoniana (de orden $k-1$).
\end{itemize}
\end{block}
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
\begin{block}{Definición:}
Un campo multivectorial de grado $m$ en una variedad $M$ (con $m \leq n = \text{dim } M$) es una sección del fibrado $\wedge^{m}(TM)\rightarrow M$ (que es un campo tensorial antisimétrico de grado $m$ en $M$ y contravariante).
\end{block}
El conjunto de los campos multivectoriales de grado $m$ en una variedad $M$ lo denotamos por $\mathfrak{X}^{m}(M)$.
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Para todo $p \in M$, hay un entorno $U_{p} \subset M$ y campos vectoriales $X_{1}, \ldots , X_{r} \in \mathfrak{X}(U_{p})$, con $m \leq r \leq \text{dim } M$, tal que:
\pause
\[
\mathbf{X}|_{U_{p}}=
\sum_{1 \leq i_{1} < \ldots < i_{m} \leq r}
f^{i_{1} \ldots i_{m}} X_{i_{1}} \wedge \ldots \wedge X_{i_{m}}
\text{ ; con } f^{i_{1} \ldots i_{m}} \in C^{\infty}(U_{p})
\]
\end{frame}
%---------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Sea $\mathbf{X} \in \mathfrak{X}^{m}(M)$ un campo multivectorial:
\begin{block}{Definición:}
\begin{itemize}
	\item $\mathbf{X}$ es homogéneo si existen $X_{1}, \ldots , X_{m} \in \mathfrak{X}(M)$ tal que $\mathbf{X} = X_{1} \wedge \ldots \wedge X_{m}$.
	\item $\mathbf{X}$ es locálmente homogéneo si, para todo $p \in M$, existen $U_{p} \in M$ y $X_{1}, \ldots , X_{m} \in \mathfrak{X}(U_{p})$ tal que $\mathbf{X}|_{U_{p}} = X_{1} \wedge \ldots \wedge X_{m}$.
\end{itemize}
\end{block}	
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Todo campo multivectorial $\mathbf{X} \in \mathfrak{X}^{m}(M)$ define una contracción con las formas diferenciales $\Omega \in \Omega^{k}(M)$.
\[
i_{\mathbf{X}}\Omega|_{U_{p}} =
\sum_{1 \leq i_{1} < \ldots < i_{m} \leq r}
f^{i_{1} \ldots i_{m}} i_{X_{1} \wedge \ldots \wedge X_{m}}\Omega =
\]
\[
= \sum_{1 \leq i_{1} < \ldots < i_{m} \leq r}
f^{i_{1} \ldots i_{m}} i_{X_{1}} \ldots  i_{X_{m}}\Omega
\]
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Si $\mathbf{X} \in \mathfrak{X}^{m}(M)$, la derivada de Lie de $\Omega \in \Omega^{k}(M)$ es:
\[
\mathcal{L}_{\mathbf{X}}\Omega = d(i_{\mathbf{X}}\Omega)- (-1)^{m}
i_{\mathbf{X}}d \Omega
\]
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Sea $(M, \Omega)$ una variedad multisimpléctica de orden $k$:
\begin{block}{Definición:}
Un campo multivectorial $\mathbf{X} \in \mathfrak{X}^{m}(M)$ con $(m < k)$ es un campo multivectorial locálmente Hamiltoniano si $\mathcal{L}_{\mathbf{X}}\Omega =0$, es decir, $i_{\mathbf{X}}\Omega \in \Omega^{k-m}(M)$ es una forma cerrada. 
\end{block}
\pause
Por lo tanto, para todo $p \in M$, existe $U \in M$ y $\zeta \in \Omega^{k-m-1}(U)$ tal que $i_{\mathbf{X}}\Omega = d \zeta$ (en U).
\pause
\begin{block}{Definición:}
$\zeta \in \Omega^{k-m-1}(U)$ es una forma locálmente Hamiltoniana para $\mathbf{X}$.
\end{block}
\end{frame}
%----------------------------------------
\begin{frame}{Estructuras Hamiltonianas en variedades multisimplécticas.}
Sea $(M, \Omega)$ una variedad multisimpléctica de orden $k$:
\begin{block}{Definición:}
$\mathbf{X} \in \mathfrak{X}^{m}(M)$ es un campo multivectorial Hamiltoniano si $i_{\mathbf{X}}\Omega \in \Omega^{k-m}(M)$ es una forma exacta; es decir, existe $\zeta \in \Omega^{k-m-1}(M)$ tal que $i_{\mathbf{X}}\Omega = d \zeta$.
\end{block}
En este caso $\zeta \in \Omega^{k-m-1}(M)$ es la denominada forma Hamiltoniana para $\mathbf{X}$.
\end{frame}
%----------------------------------------
...
i(\rho_{*}V_{k}\wedge, \ldots , \wedge \rho_{*}V_{1})\alpha
\]
para:\\
